...
- O método `createQuery` da API retorna `{id, sql}`; `query.ts:88-95` extrai isso e executa localmente. A `id` ainda não é usada para fetch de answer/chart (vide TODO em `query.ts:128-133` — "POST /v1/queries/:id/answer would go here").
...
- SDK: `@clearsoft/easysql-sdk` no npm, consumido via dep semver `^1.0.0`.
